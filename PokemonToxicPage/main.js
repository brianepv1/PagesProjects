...
// http://localhost:3000/ - display home page
app.get(['/'], (request, response) => {
	// Create CSRF token
	let token = cryptography.randomBytes(20).toString('hex');
	// Store token in session
	request.session.token = token;
	// User is not logged in, render home template
	response.render('index.html', { token: token, logged: request.session.account_loggedin});
});


// http://localhost:3000/login - display login page
app.get(['/', '/login'], (request, response) => isLoggedin(request, settings => {
	...
	response.render('login.html', { token: token, logged: request.session.account_loggedin });
}));

// http://localhost:3000/events/ - Events page
app.get('/events', (request, response) => {
	// Render forgot password template and output message
	response.render('events.html', {logged: request.session.account_loggedin});
});

// http://localhost:3000/leaderboard/ - Leaderboard page
app.get('/leaderboard', (request, response) => {
	// Render forgot password template and output message
	//query SELECT name,online,level,experience,vocation,promotion, world_id FROM players WHERE players.world_id LIKE ? AND players.deleted = 0 AND players.group_id < ? AND name NOT IN(?) ORDER BY level DESC, experience DESC LIMIT 50'
	// SELECT name, online, level, experience,vocation,promotion,world_id FROM players WHERE players.world_id = "0" AND players.deleted = "0" AND players.group_id < "3" ORDER BY level DESC, experience DESC LIMIT 50
	connection.query('SELECT name, online, level, experience,vocation,promotion,world_id FROM players WHERE players.world_id = "0" AND players.deleted = "0" AND players.group_id < "3" ORDER BY level DESC, experience DESC LIMIT 50', (error, accounts, fields) => {
		//console.log("Characters: ",accounts[0].name);
		let data = JSON.parse(JSON.stringify(accounts));
		console.log(data);
		// Render home page
		response.render('leaderboard.html', { accounts: data, logged: request.session.account_loggedin });
	});
});
...
app.post(['/', '/login'], (request, response) => init(request, settings => {
	...
	if (username && password) {
		...
		connection.query('SELECT * FROM accounts WHERE name = ? AND password = ?', [username, hashedPassword], (error, accounts) => {
			...
			if (accounts.length > 0) {
				...
				request.session.account_username = accounts[0].name;
				request.session.account_nickname = accounts[0].nickname;
				...
				if (request.body.rememberme) {
					...
					connection.query('UPDATE accounts SET rememberme = ? WHERE name = ?', [hash, username]);
				}
...
app.post('/register', (request, response) => init(request, settings => {
	...
	if (username && password && email) {
		...
		connection.query('SELECT * FROM accounts WHERE name = ? OR email = ?', [username, email], (error, accounts, fields) => {
			...
			if (accounts.length > 0) {
				...
				connection.query('INSERT INTO accounts (name, password, email, activation_code, lastip) VALUES (?, ?, ?, ?, ?, ?)', [username, hashedPassword, email, activationCode, ip], () => {
				...
				// Insert account
				console.log("Estoy en insert account");
				console.log("Username", username);
				console.log("Pass", hashedPassword);
				console.log("Email", email);
				connection.query('INSERT INTO accounts (name, password, email) VALUES (?, ?, ?)', [username, hashedPassword, email], (error, result) => {
...
app.get('/home', (request, response) => isLoggedin(request, settings => {
	...
	connection.query('SELECT id, name, online, level, sex, account_id FROM players WHERE account_id = ?', [request.session.account_id], (error, accounts, fields) => {
		//console.log("Characters: ",accounts[0].name);
		let data = JSON.parse(JSON.stringify(accounts))
		console.log(data);
		// Render home page
		response.render('home.html', { accounts: data, role: request.session.account_role, nickname: request.session.account_username , logged: request.session.account_loggedin });
	});

	//console.log("Id: ", request.session.account_id);
	//console.log("name: ", request.session.account_username);
	//console.log("Nickname: ", request.session.account_nickname);
	//console.log("Session: ", request.session);
	//response.render('home.html', { nickname: request.session.account_nickname, role: request.session.account_role });
}, () => {
	// Redirect to login page
	response.redirect('/');
}));

// http://localhost:3000/create_character - display the characters page
app.get('/create_character', (request, response) => isLoggedin(request, settings => {
	// Render character template
	response.render('create_character.html', { nickname: request.session.account_nickname, role: request.session.account_role, logged: request.session.account_loggedin });
}, () => {
	// Redirect to login page
	response.redirect('/');
}));

// http://localhost:3000/create_character -  characters page
app.post('/create_character', (request, response) => isLoggedin(request, settings => {
	// Render character template
	let characterName = request.body.characterName;
	let sex = request.body.sex;
	let accountId = request.session.account_id;
	if(characterName){

		connection.query('SELECT name FROM players WHERE name = ?', [characterName], (error, accounts) => {
			// Output msg
			let msg = '';
			// If character name exist, send an error
			if (accounts.length > 0) {
				msg = 'This Character name exist, please change your character name!';
				//response.send('This Character name exist, please change your character name!');
				response.render('create_character.html', { msg: msg});
			} else {

				connection.query('INSERT INTO players (name, group_id, account_id, level, town_id, sex) values (?, 1, ?, 1, 1, ?) ', [characterName, accountId, sex]);

				response.redirect('/home');
				
			}
			// Render create character template
		});
	}
...
app.get('/profile', (request, response) => isLoggedin(request, settings => {
	...
	connection.query('SELECT * FROM accounts WHERE name = ?', [request.session.account_username], (error, accounts, fields) => {
		// Format the registered date
		console.log(accounts[0]);
		accounts[0].registered = new Date(accounts[0].registered).toISOString().split('T')[0];
		// Render profile page
		response.render('profile.html', { account: accounts[0], role: request.session.account_role, logged: request.session.account_loggedin });
	});
}, () => {
	// Redirect to login page
	response.redirect('/');
...
app.get('/edit_profile', (request, response) => isLoggedin(request, settings => {
	...
	connection.query('SELECT * FROM accounts WHERE name = ?', [request.session.account_username], (error, accounts, fields) => {
		...
		accounts[0].registered = new Date(accounts[0].registered).toISOString().split('T')[0];
		// Render profile page
		response.render('edit_profile.html', { account: accounts[0], role: request.session.account_role });
	});
}, () => {
	// Redirect to login page
	response.redirect('/');
...
app.post('/edit_profile', (request, response) => isLoggedin(request, settings => {
	...
	if (password != cpassword) {
		...
		connection.query('SELECT * FROM accounts WHERE name = ?', [username], (error, accounts, fields) => {
			...
			if (accounts.length > 0 && username != request.session.account_username) {
				// Username exists
				response.render('edit_profile.html', { account: accounts[0], msg: 'Username already exists!', role: request.session.account_role });
			} else {
				// Update account with new details
				connection.query('UPDATE accounts SET name = ?, password = ?, email = ?, activation_code = ? WHERE name = ?', [username, hashedPassword, email, activationCode, request.session.account_username], () => {
					...
					connection.query('SELECT * FROM accounts WHERE name = ?', [username], (error, accounts, fields) => {
						...
						response.render('edit_profile.html', { account: accounts[0], msg: msg, role: request.session.account_role });
					});
				});
			}
		});
...
// http://localhost:3000/delete_character/:id - delete character password
app.get('/delete_character/:account_id/:id', (request, response) => isLoggedin(request, settings => {
	// Make sure the params are specified

		if(request.session.account_id == request.params.account_id){
			if (request.params.id) {
				connection.query('DELETE FROM players WHERE id = ?', [request.params.id]);
				response.redirect('/home');
			}
		}else{
			response.redirect('/home');
...
app.get('/news', (request, response) => {
	// Render forgot password template and output message
	response.render('news.html', { logged: request.session.account_loggedin });
});

// http://localhost:3000/Download/ - Download page
app.get('/download', (request, response) => {
	// Render forgot password template and output message
	response.render('download.html', { logged: request.session.account_loggedin });
});
